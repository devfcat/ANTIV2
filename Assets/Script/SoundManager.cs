...
public class SoundManager : MonoBehaviour
{
    ...
    private AudioSource specialSePlayer; // Sfx AudioSource, SoundManager 세 번째 자식이어야 함
    ...
    private void GetVolumePref()
    {
        ...
        else
        {
            ...
            specialSePlayer.volume = SfxVolume;
    ...
    private void Init()
    {
        ...
        specialSePlayer = GetComponentsInChildren<AudioSource>()[2];
    }
    ...
    /// se Clip 선택 후 재생 메소드
    /// </summary>
    /// <param name="se">재생할 se 파일 인덱스</param>
    public void PlaySe(SFX sfx)
    {
        if (specialSePlayer.volume > 0) // 음소거 시 재생하지 않음
        {
            Debug.Log(sfx + " played");
            specialSePlayer.clip = sfxList[(int)sfx];
            specialSePlayer.PlayOneShot(specialSePlayer.clip);
        }
    }
